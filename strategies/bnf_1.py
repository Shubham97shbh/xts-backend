...
@shared_task
def check_and_trail_sl_sell(exchange_instrument_id, strike, side, process_id):
    global CE_MULTIPLE, OTM_GAP, PE_MULTIPLE, CURRENT_PE_MARGIN, CURRENT_CE_MARGIN, CURRENT_PNL, \
        EMERGENCY_STOP, CE_REENTRY, PE_REENTRY, SL_PERCENTAGE, QTY, TRADE, TRAIL_SL, CUR_CE_MULTIPLE, \
        CUR_PE_MULTIPLE, PORTFOLIO_RISK, SL_FLAG, TRADE_TYPE
...
@shared_task
def check_and_trail_sl_buy(exchange_instrument_id, strike, side, process_id):
    global CE_MULTIPLE, OTM_GAP, PE_MULTIPLE, CURRENT_PE_MARGIN, CURRENT_CE_MARGIN, CURRENT_PNL, \
        EMERGENCY_STOP, CE_REENTRY, PE_REENTRY, SL_PERCENTAGE, QTY, TRADE, TRAIL_SL, CUR_CE_MULTIPLE, \
        CUR_PE_MULTIPLE, PORTFOLIO_RISK, SL_FLAG, TRADE_TYPE
...
@app.task
def place_trade(strike, side, exchange_instrument_id, process_id, trade_type):
    global CE_MULTIPLE, OTM_GAP, PE_MULTIPLE, CURRENT_PE_MARGIN, CURRENT_CE_MARGIN, CURRENT_PNL, \
        EMERGENCY_STOP, CE_REENTRY, PE_REENTRY, SL_PERCENTAGE, QTY, TRADE, TRAIL_SL, CUR_CE_MULTIPLE, \
        CUR_PE_MULTIPLE, PORTFOLIO_RISK, SL_FLAG, TRADE_TYPE
    try:
        ...
        SharedObject.modify_value(
            update_value, process_id, nested_key=f'trade_side_dic.{side}')
        ...
        if TRADE == 'SELL':
            ...
            if side == "CE":
                while CE_REENTRY > 0 and EMERGENCY_STOP:
                    ...
                    exchange_instrument_id, _, strike = get_atm(
                        xt, "BANKNIFTY", atm)
            ...
            if side == "PE":
                while PE_REENTRY > 0 and EMERGENCY_STOP:
                    ...
                    spot = Data.get_ltp(exchange_instrument_id)
                    atm = int(round(spot / 100)) * 100
                    # exchange_instrument_id = get_atm(xt, "BANKNIFTY", atm)
                    _, exchange_instrument_id, strike = get_atm(
                        xt, "BANKNIFTY", atm)
                    order_res = xt.place_order(exchangeSegment=xt.EXCHANGE_NSEFO,
                                               exchangeInstrumentID=exchange_instrument_id,
                                               productType=xt.PRODUCT_NRML,
                    ...
                    create_logs(logger,  process_id,
                                f"{datetime.now()} | Reentry Order Placed: {strike} | Entry Price: {entry_price} | SL: {sl_point}")
                    PE_REENTRY -= 1
                    check_and_trail_sl_sell.delay(exchange_instrument_id, strike, side, str(process_id))
...
def bnfv_1(process_id, data: dict):
    ...
    try:
        ...
        async_result = group(place_trade.s(strike_ce, "CE", ce_instrument_id, process_id, TRADE_TYPE),
                             place_trade.s(strike_pe, "PE", pe_instrument_id, process_id, TRADE_TYPE)).apply_async()
