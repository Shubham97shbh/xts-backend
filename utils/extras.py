...
def check_finished(logs:list)->str:
    '''
    Cheking the value is already square off or not
    logs: Log generated by orders
    '''
    for log in logs:
        if 'FININSHED' in log.upper() or 'ERROR IN PLACING ORDER' in log.upper():
            return True
    return False
...
def get_atm(xt, index: str, atm_strike) -> list[str]:
    ...
    for gap in range(-200, 300, 100):
        ...
        print('Rates in the market', ce_rate,
              pe_rate, exchange_instrument_id_ce)
...
def get_trade_ltp(xt, order_id):
    response = xt.get_trade(clientID="*****")
